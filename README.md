Live : https://job-search-ai-ten.vercel.app/


# JobSearch.ai 💼

> An AI-powered, web-wide job discovery and application assistant that helps candidates find relevant jobs across company career pages, ATS platforms, and popular job boards — while keeping the actual application fully under the candidate's control.

[![Next.js](https://img.shields.io/badge/Next.js-TypeScript-black?logo=next.js)](https://nextjs.org)
[![FastAPI](https://img.shields.io/badge/FastAPI-Python-green?logo=fastapi)](https://fastapi.tiangolo.com)
[![MongoDB](https://img.shields.io/badge/MongoDB-Database-green?logo=mongodb)](https://mongodb.com)
[![Gemini](https://img.shields.io/badge/AI-Gemini-blue)](https://ai.google.dev)

---

## What is JobSearch.ai?

JobSearch.ai searches across the public web to help candidates discover jobs matching their role, skills, experience, and location.

It can discover listings from:

- 🏢 Company career pages
- 💼 LinkedIn
- 🔎 Indeed
- 🎓 Unstop
- 🧑‍💻 Internshala
- 💼 Naukri / Foundit
- 👾 Monster
- 🌱 Wellfound
- 🏗️ Greenhouse
- 🚀 Lever
- And other publicly accessible job sources

The system preserves **real job and application URLs** instead of generating fake or Google-search fallback links.

### 🔒 Human-Controlled Applications

JobSearch.ai **never automatically applies for a candidate**.

```text
Discover Job
     ↓
Analyze & Match
     ↓
Open Real Job / Application Page
     ↓
Candidate Applies Manually
     ↓
"I Applied" → APPLIED
"Didn't Apply" → NOT_APPLIED

MIT — see [LICENSE](LICENSE)

---

preview :
<img width="1900" height="862" alt="Screenshot (375)" src="https://github.com/user-attachments/assets/1492c2ca-29ca-404a-b151-ea6343c7e6d1" />
<img width="1890" height="860" alt="Screenshot (376)" src="https://github.com/user-attachments/assets/226030ab-a3f1-4fa7-a413-2030a21bcb9f" />
<img width="1885" height="874" alt="Screenshot (377)" src="https://github.com/user-attachments/assets/81df9ea2-4966-415f-82e4-f7553ca362d6" />




*Built with ❤️ for working professionals & students *
